...
- **[NEW] アンケート自動生成（AD-005）**: 分析結果から上位意見を抽出し、アンケート（質問・選択肢）をドラフト作成
...
- **[NEW] アンケート生成プロンプト**: 抽出された意見群から、公平で分かりやすい質問文と選択肢を生成
...
- プッシュ通知配信
- 4択表示（上位3意見 + 該当なし）
- **[NEW] AI生成アンケートの配信**: 管理者が承認したAI生成ドラフトを配信
- ワンタップ投票処理
- ポイント付与処理
...
- `poll_management.py`: アンケート配信管理（AD-009）
  - 新規作成（手動）
  - **[NEW] AI提案の確認・承認**: 自動生成されたドラフトの修正・公開設定
