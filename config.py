...
# システムプロンプト（アンケート生成フェーズ）
SYSTEM_PROMPT_POLL_GENERATION = """市民の意見（クラスタリング結果）から、多くの市民が関心を持ちそうなアンケートを作成してください。

以下のJSON形式で出力してください:
{
  "question": "アンケートの質問文（40文字以内）",
  "options": [
    "選択肢1（20文字以内）",
    "選択肢2（20文字以内）",
    "選択肢3（20文字以内）",
  ...
  "description": "アンケートの補足説明（100文字以内）"
}

要件:
- 選択肢は必ず4つ作成してください。
- 4つ目の選択肢は「その他」や「該当なし」など、否定的な意見やその他の意見も拾えるものにしてください。
- 質問は公平で中立的な表現にしてください。
- JSONのみを出力し、余計な説明は不要です。"""
